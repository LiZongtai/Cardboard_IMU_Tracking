...
void HeadTracker::Resume() {
  CARDBOARD_LOGI("HeadTracker Resume");
  is_tracking_ = true;
  RegisterCallbacks();
}
...
void HeadTracker::OnAccelerometerData(const AccelerometerData& event) {
    ...
    //    !   <-  O   ->   !             !   <-  O   ->   !
    //    !________________!             !________________!
//  CARDBOARD_LOGI("OnAccelerometerData = %f, %f, %f", event.data[0],event.data[1],event.data[2]);
  if (!is_tracking_) {
    return;
  }
...
void HeadTracker::OnGyroscopeData(const GyroscopeData& event) {
    ...
    //    !   <-  O   ->   !             !   <-  O   ->   !
    //    !________________!             !________________!
//  CARDBOARD_LOGI("OnGyroscopeData = %f, %f, %f", event.data[0],event.data[1],event.data[2]);
  if (!is_tracking_) {
    return;
  }
